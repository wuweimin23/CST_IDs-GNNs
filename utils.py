...
def load_dgl():
    ...
    label = data[0].ndata['label']
    src, dst = data[0].edges()
    feat = np.array(feat)
    label = np.array(label)
    src = np.array(src)
    dst = np.array(dst)
    one = np.ones(src.shape)
    coo = sp.coo_matrix((one, (src, dst)), shape=(feat.shape[0], feat.shape[0]), dtype=np.int)
    return coo, feat, label


def get_adj(dataset, require_lcc=True):
    print('reading %s...' % dataset)
    # _A_obs, _X_obs, _z_obs = load_npz(r'data/%s.npz' % dataset)
    _A_obs, _X_obs, _z_obs = load_dgl()
